...
def get_data():
	...
	# get the highest, lowest, average, std dev, and number of prices
	sql = '''SELECT MAX(high), MIN(low), AVG(close), STDDEV(close), COUNT(*)   
			FROM tsla_stock 
			WHERE date >= %s AND date <= %s'''
	sql_tuple = (startDate, endDate)
	cursor.execute(sql, sql_tuple)
	...
	# save those vars accordingly
	stock_price_highest = result[0][0]
	stock_price_lowest = result[0][1]
	stock_price_average = result[0][2]
	stock_price_stddev = result[0][3]
	count_rows = result[0][4]
	
	# second, find the median based on whether there are
